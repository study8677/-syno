...
const CommentForm: React.FC<CommentFormProps> = ({ onSubmit, currentUser, replyTo, onCancelReply }) => {
    ...
    if (!currentUser) {
        return <div className="bg-syno-dark-secondary p-4 rounded-lg text-center text-syno-text-secondary border border-syno-border">{t('comment.loginPrompt.prefix')} <a href="/#/login" className="text-syno-primary hover:underline">{t('comment.loginPrompt.loginLink')}</a> {t('comment.loginPrompt.suffix')}</div>;
    }
        ...
        <form onSubmit={handleSubmit} className="bg-syno-dark-secondary p-4 rounded-lg border border-syno-border">
            {replyTo && (
                <div className="text-sm text-syno-text-secondary mb-2">
                    {t('comment.replyingTo')} {replyTo.authorName}:
                    <button type="button" onClick={onCancelReply} className="ml-2 text-red-500 hover:underline">[{t('common.cancel')}]</button>
                </div>
            )}
            <div className="flex items-center space-x-3">
                ...
                >
                    {isSubmitting ? t('comment.generating') : t('comment.generateButton')}
                </button>
            </div>
        </form>
    );
...
export const CommentSection: React.FC<CommentSectionProps> = ({ entityType, entityId, question, comments, onComment, ...rest }) => {
    ...
    const handleCommentSubmit = async (persona: PersonaProfile, parentId: number | null) => {
        ...
        const constructedPrompt = language === 'en' ? `
          You are posting a comment on an AI Q&A community called "Syno".
          Please generate a concise, insightful, and natural-sounding comment based on the following context.
          Your persona for this comment is: "${persona.name}", whose style is: "${persona.description}".

          **Context:**
          - **Question:** "${question.title}"
          - **Replying to:** "${parentComment ? parentComment.content : 'A comment on the main question'}"

          Directly output the generated comment content without any additional explanations or introductory phrases.
        ` : `
